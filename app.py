...
def preferCreate():
    ...
    if not os.path.exists(path_anime):
        print(" DEBUG: DOWNLOADS DIRECTORY NOT EXIST CANNOT GET LAST ANIME DOWNLOADS")
    else:
        print(path)
        ...
        for anime in files:
            if '.mp4' in anime:
                ##print("DEBUG:" + str(anime))
                info = rgx.split(regex, anime)
                info = [x for x in info if x] 
                ##print(" DEBUG: NOME ANIME PREFER: " + str(info[0]))
                if(info[0] in EpDict.keys()):
                    temp_ep = info[1]
                    if(int(EpDict[info[0]]) < int(temp_ep)):
                        ##print("DEBUG: VALORE DICT KEY: " + str(EpDict[info[0]]))
                        ##print("DEBUG: STRINGA TEST EP PREFERITI IF " + str(info[1]))
                        EpDict[info[0]] = temp_ep
                else:
                    ##print("DEBUG: STRINGA TEST EP PREFERITI ELSE " + str(info[1]))
                    EpDict[info[0]] = info[1]
                title = info[0]
                if title not in arrayTitle:
                    arrayTitle.append(title)
                ##print("DEBUG:" + str(arrayTitle))
...
if __name__ == "__main__":
    ...
    if "sliderDA2" not in st.session_state:
        ...
        if(st.session_state.sliderDA2 != 0):
            ##print("TEST1 SLIDER2")
            if(st.session_state.sliderA2 < st.session_state.sliderDA2):
                ##print("TEST2 SLIDER2")
                if(st.session_state.sliderDA2 < st.session_state.maxEp):
                    ##print("TEST3 SLIDER2")
                    st.session_state.sliderA2 = st.session_state.sliderDA2 + 3
                else:
                    st.session_state.sliderA2 = st.session_state.sliderDA2
        if(st.session_state.sliderDA2 == 0):
            st.session_state.sliderA2 = 0
    ...
    def main():
        ...
        with col1:
                ...
                if(st.session_state.name_input != st.session_state['name'] and st.session_state['name'] != 'value' and st.session_state.name_input != ''):
                    ##print('DEBUG: NAME UGUALE A ' + str(st.session_state.name_input))
                    st.session_state.checkSelect = False
                if(st.session_state.checkSelect == False):
                    st.session_state['name'] = st.session_state.name_input
                    st.session_state.disable = True
                    ##print("DEBUG: VALORE NAME INPUT IN MAIN R.187 " + st.session_state.name_input)
                    EpSlider1(col1)
                else:
                    st.session_state.disable = False
        ...
        with cardCol1:
            ...
            if 'B0' in st.session_state:
                ...
                ##print("DEBUG: text1 " + text)
                if st.button(fr'{text}'):
                    ##print("DEBUG: entrato primo button clicked")
    ...
    def getEp(start: int,end: int,_anime,image,_cardCol1, _cardCol2, _cardCol3, _cardcol4, _arrayB: list):
        ...
        for ep in episodes:
            ...
            if(i < 4):
                ##print(fr" DEBUG: entrato if i: {i}")
                None
            else:
                st.session_state['i'] = 0
                st.session_state['counter'] = 0
                st.session_state[f'{i}'] = CardObj(_anime.getName(), "Ep." + str(start), image)
                ##print(fr" DEBUG:entrato else i: {i}")
            ##print("DEBUG: valore i:  "i)
            print(f"DEBUG: Downloading episode {ep.number}.")
            name_file = fr"{_anime.getName()}{start}"
            ep.download(name_file, "AnimeDownloads")
            print(f" DEBUG: Download completed.")
            st.session_state['counter'] += 1
            ##print("DEBUG: GET EP COUNTER " + str(st.session_state['counter']))
            ...
            ##print("DEBUG: INSERITO IN ARRAY B")
            if f'{i}' not in st.session_state:
                st.session_state[f'{i}'] = CardObj(_anime.getName(), "Ep." + str(start), image)
                ##print(" DEBUG: ENTRATO IF NOT")
                ##print(f'DEBUG: {i}')
                ##print("DEBUG: " str(st.session_state[f'{i}']))
            else:
                st.session_state[f'{i}'] = CardObj(_anime.getName(), "Ep." + str(start), image)
                ##print("DEBUG: ENTRATO ELSE NOT")
                ##print(f'DEBUG: {i}')
                ##print(DEBUG: str(st.session_state[f'{i}'])) 
            if(i == 0):
                _cardCol1.empty()
    ...
    def getAnimeInfo(cardCol1, cardCol2, cardCol3, cardcol4, arrayB):
        ...
        if(anime_info == []):
            ## TENTATIVE N.2
            ##print("DEBUG: ANIME SESSION NAME FUNC GETANIMEINFO NAME VAL: " + str(st.session_state["name"]))
            anime_info = aw.find(st.session_state['name'])
            if(anime_info == []):
                st.write("Anime con quel nome non esistente")
                return
        ##print("DEBUG:" + str(anime_info))
    ...
    def EpSlider1(col1):
        ##print('DEBUG: ENTRATO EP SLIDER 1')
        if(st.session_state['name'] != 'value' and st.session_state['name'] != ''):
            ##print('DEBUG: Valore di name e ' + str(st.session_state['name']))   
            anime_info = aw.find(st.session_state['name'])
            ##print('DEBUG: FUNZIONE ANIMEWORLD RITORNO: ' + str(anime_info))
            if(anime_info == []):
                ## TENTATIVE N.2
                ##print("DEBUG: ANIME SESSION NAME FUNC EPSLIDER1 NAME VAL: " + str(st.session_state["name"]))
                anime_info = aw.find(st.session_state['name'])
                if(anime_info == []):
                    st.write("Anime con quel nome non esistente")
                    return
            ##print("DEBUG: EPISODI MASSIMI ANIME: " + str(anime_info[0]['episodes']))
            max_episodes = anime_info[0]['episodes']
            max_episodes = int(max_episodes)
            if "maxEp" not in st.session_state:
                st.session_state.maxEp = max_episodes
            ##print("DEBUG: EPISODI MASSIMI APP.PY " + str(anime_info[0]['episodes']))
            with col1.container():
            ...
            ##print('DEBUG: ELSE valore di name e value')
            None
        return
    
    def EpSlider2(col1, last_ep):
        ##print('DEBUG: ENTRATO EP SLIDER 2')
        if(st.session_state['name'] != 'value' and st.session_state['name'] != ''):
            ##print('DEBUG: Valore di name e ' + str(st.session_state['name']))   
            anime_info = aw.find(st.session_state['name'])
            if(anime_info == []):
                ## TENTATIVE N.2
                ##print("DEBUG: ANIME SESSION NAME FUNC EPSLIDER2 NAME VAL: " + str(st.session_state["name"]))
                anime_info = aw.find(st.session_state['name'])
                if(anime_info == []):
                    st.write("Anime con quel nome non esistente")
                    return
            ##print("DEBUG: ANIME INFO IN EPSLIDER2 " + str(anime_info))
            ##print('DEBUG: FUNZIONE ANIMEWORLD RITORNO: ' + str(anime_info))
            ##print("DEBUG: EPISODI MASSIMI ANIME: " + str(anime_info[0]['episodes']))
            max_episodes = anime_info[0]['episodes']
            max_episodes = int(max_episodes)
            ...
            ##print("DEBUG: EPISODI MASSIMI APP.PY " + str(anime_info[0]['episodes']))
            with col1.container():
                ##print("DEBUG: LAST EP UGUALE A " + str(last_ep))
                ep = int(last_ep)
                st.slider(label='DA', min_value=0, value= ep + 1, max_value=max_episodes, step=1,format=None, key = 'sliderDA2', on_change=setStart2())
                ##print("VALORE SLIDER DA " + str(sliderDA))
            with col1.container():
                ##print('DEBUG: CREATION SLIDER A EPSL2')
                st.slider(label='A', min_value=0, value= int(ep+4), max_value=max_episodes, step=1,format=None, key = 'sliderA2', on_change=setEnd2())
                ##print("DEBUG: valore sliderDA2: " + str(st.session_state.sliderDA2) + " valore SLiderA2: " + str(st.session_state.sliderA2))
        else:
            ##print('DEBUG: ELSE valore di name e value')
            None
        return
    ...
    def startFunc(path, name):
        ##print("DEBUG: PATH BUTTON INSIDE START METHOD:" + path)
        path = Path.joinpath(Path.cwd(), path)
        startfile(path)
        ##print("DEBUG: AVVIATO FILE " + str(path))
    ...
    def setStart():
        ...
        ##print("DEBUG: START: " + str(st.session_state['start']))
        return
    ...
    def setEnd():
        ...
        ##print("DEBUG: END: " + str(st.session_state['end']))
        return
    ...
    def setName():
        ...
        ##print("DEBUG: SETNAME name: " + st.session_state['name'])
        return

    def CreateSelect():
        ##print("DEBUG: CREATESELECTE value disable:  " + str(st.session_state.disable))
