            ...
            while True:
                try:
                    ...
                    if not attempt:
                        ...
                        print("TEST PRINT RAISE E UTILITY ")
                        raise e    
                    attempt = True
...
def find(keyword: str) -> List[Dict]:
    ...
    ##print("DATI DA UTILITY RESPONSE: " + str(res))
    ##print("RESPONSE JSONATA: " + str(res.json()))
    data = res.json()
    ...
    print("TEST VALORE EPIDOSI ")
    link = f"https://www.animeworld.so/play/{data[0]['link']}.{data[0]['identifier']}"
    print(link)
    if(data[0]['episodes'] == None):
        URL = link
        page = rq.get(URL)
        soup = BeautifulSoup(page.content,'html.parser')
        range_ep = soup.find_all("span", class_="rangetitle")
        lenght = len(range_ep)
        num_ep = range_ep[lenght - 1].text
        num_ep = num_ep.split()
        lenght = len(num_ep)
        num_ep = num_ep[lenght - 1]
        print(num_ep[lenght - 1])
    ...
    return [
        ...
        "episodes": int(elem["episodes"]) if elem["episodes"] is not None else num_ep,
        "state": elem["state"],
        "story": elem["story"],
