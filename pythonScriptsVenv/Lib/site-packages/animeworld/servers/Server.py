...
from streamlit_app import getArrayCards
from streamlit_app import getCounter
            ...
            try:
                for chunk in r.iter_bytes(chunk_size = 524288):
                    if chunk: 
                        ...
                        if(i == 0):
                          print("Valore di i: " + str(i))
                          counter = getCounter()
                          cards[counter].empty()
                          with cards[counter].container():
                            download_bar = st.progress(int(0),'Download in corso...')
                          i = 1
                        
                        print('counter: ' + str(counter))
                        text = 'Download in corso...      Velocità: ' + str(round((len(chunk) / (time.time() - step) if (time.time() - step) != 0 else 0) / pow(10,6), 2)) + "MB/s"
                        print("Lunghezza totale: " + str(total_length))
                        print("Lunghezza scaricata: " + str(current_lenght))
                        percentage = (current_lenght / total_length) * 100
                        download_bar.progress(int(percentage), text)
                        if(percentage >= 100):
                            download_bar.empty()
